...
class OriginalYiiRbacTest extends TestCase
{
    ...
    public function testAssignRule()
    {
        ...
        $auth->removeAll();
        $role = $auth->createRole('Reader');
        $role->ruleName = 'tests\unit\ActionRule';
        $auth->add($role);
        $auth->assign($role, $userId);
        $this->assertTrue($auth->checkAccess($userId, 'Reader', ['action' => 'read']));
        $this->assertFalse($auth->checkAccess($userId, 'Reader', ['action' => 'write']));
        // using DI
        \Yii::$container->set('write_rule', ['class' => 'tests\unit\ActionRule', 'action' => 'write']);
        \Yii::$container->set('delete_rule', ['class' => 'tests\unit\ActionRule', 'action' => 'delete']);
        \Yii::$container->set('all_rule', ['class' => 'tests\unit\ActionRule', 'action' => 'all']);
